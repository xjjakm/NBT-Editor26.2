	...
	public void register(LiteralArgumentBuilder<FabricClientCommandSource> builder, String path) {
		builder.then(literal("newuuids").executes(context -> {
			...
			if (attributes.isEmpty()) {
				...
				ref.saveItem(item, () -> {
					if (MainUtil.client.player != null)
						MainUtil.client.player.sendSystemMessage(TextInst.translatable("nbteditor.attributes.new_uuids.success"));
				});
			}
			return Command.SINGLE_SUCCESS;
		})).executes(context -> {
			NBTReference.getReference(ATTRIBUTES_FILTER, false, ref -> MainUtil.client.gui.setScreen(new AttributesScreen<>(ref)));
			return Command.SINGLE_SUCCESS;
		});
	}
