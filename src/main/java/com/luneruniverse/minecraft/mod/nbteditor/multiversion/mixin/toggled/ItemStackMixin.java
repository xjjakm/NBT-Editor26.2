...
public class ItemStackMixin implements MVItemStackParent {
	...
	public boolean contains(MVComponentType<?> type) {
		return ((ItemStack) (Object) this).has((DataComponentType<?>) type.getInternalValue());
	}
	...
	public <T> T apply(MVComponentType<T> type, T defaultValue, UnaryOperator<T> applier) {
		return ((ItemStack) (Object) this).update((DataComponentType<T>) type.getInternalValue(), defaultValue, applier);
	}
	@SuppressWarnings("unchecked")
	@Override
	public <T, U> T apply(MVComponentType<T> type, T defaultValue, U change, BiFunction<T, U, T> applier) {
		return ((ItemStack) (Object) this).update((DataComponentType<T>) type.getInternalValue(), defaultValue, change, applier);
	}
	@SuppressWarnings("unchecked")
	@Override
